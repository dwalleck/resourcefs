...
|---|---|---|---|---|---|---|---|---|
| C1 | Fixture trust preserves chain and hostname verification | valid fixture CA; matching and mismatching hostname leaves | Trust the fixture CA, request both leaves at an authorized address, and fail if the matching handshake does not complete or the mismatching handshake completes/sends a request | TLS listener’s server-side handshake outcome and request log; neither is derived from the client result | `crates/resourcefs-sources/src/http/mod.rs`: configure the client to accept invalid hostnames when extra roots are present; the mismatching listener records a completed handshake/request and `tls_certificate_binds_hostname` turns red | `crates/resourcefs-sources/tests/http_tls_contract.rs::tls_certificate_binds_hostname` | <1 min | PASS |
| C2 | Malformed fixture DER is rejected before egress | malformed DER root | Construct the test-root substrate with invalid DER beside an accept-counting listener; any successful construction or accepted socket falsifies | Literal invalid DER bytes plus listener accept count zero | `crates/resourcefs-sources/src/http/mod.rs`: ignore `Certificate::from_der` failure and build with system roots; construction succeeds and the malformed-root fence turns red | `crates/resourcefs-sources/tests/http_tls_contract.rs::malformed_additional_root_is_rejected_before_egress` | <1 min | PENDING — checkpointed-build, root-constructor slice |
