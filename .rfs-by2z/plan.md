...
**Stress fixture:** Title/body/comment current/stale/missing tags; exact request order (title/body GET→PATCH, comment parent-kind GET→comment GET→PATCH); empty/blank/Unicode/max bodies; two same-Field contenders and distinct-Field concurrency; stale ETag cache; response normalization; every approved status with body/credential sentinel; 500 GitHub cache keys plus 500 non-GitHub/collision-prefix keys (the full 1,000-object Path-Session ceiling); all non-mutable paths; source/repository grant subsets/delete. Expected: one winner/one stale conflict, exact request counts and one-field PATCH, response-vector tag, no seen entry, cache empty before adapter return, no secret/body leak, and zero egress for denials.
...
**Complexity/production scale:** Field classification/status/identity checks are $O(1)$ plus response JSON decode $O(r)$ under the HTTP fetch ceiling. Cache namespace removal scans $k \le MAX_SESSION_ARTIFACTS = 1,000$ cache entries once, so it is $O(k)$ with at most 1,000 key comparisons; no namespace index or duplicate keys are retained. Maximum accepted local cost: 50 ms to remove/verify 500 GitHub entries while preserving 500 non-GitHub entries and 100 ms to validate a maximum bounded response.

**Wall budget/phase:** Always-on title/body replacement performs one uncached GET plus one PATCH (maximum 60,000 ms); comment replacement performs parent-kind GET plus comment GET plus PATCH (maximum 90,000 ms); every request is independently bounded by 30,000 ms. Local compare/decode/cache work budget is 150 ms at production ceilings. These exact request counts preserve canonical issue/PR comment identity without claiming a nonexistent single-call parent discriminator.

**Files:** create `crates/resourcefs-sources/src/github/mutation.rs` and `crates/resourcefs-sources/tests/github_mutation_contract.rs`; modify `crates/resourcefs-core/src/mutation.rs`; `crates/resourcefs-sources/src/github/{mod.rs,wire.rs}`; `crates/resourcefs-sources/src/{compiled.rs,filesystem_mutation.rs,local.rs}`; `crates/resourcefs-core/src/session.rs`; `crates/resourcefs-sources/tests/support/tls.rs`; `crates/resourcefs-sources/tests/github_adapter_contract.rs`; `crates/resourcefs-sources/tests/configuration_contract.rs`; `crates/resourcefs-mcp/tests/{profile_contract.rs,stdio_mcp_contract.rs,architecture_contract.rs}`; `crates/resourcefs-mcp/tests/fixtures/server-profile-v1.schema.json` only if generated schema bytes change.
...
**Expected behavior:** Creation Target metadata/grants validate before network; strict issue/PR documents and non-blank comment Markdown produce only approved JSON keys; the six total mutation routes match empirical evidence; issue/PR creation sends one POST, while comment creation sends one parent-kind GET then one POST; success validates repository/parent/ID and returns canonical reference/null tag/no coverage; identical calls coalesce/repeat, conclusive 3xx/4xx permits identical caller retry, 5xx/transport/drop/wrong-or-invalid success becomes unknown and blocks, no mutating request retries/redirects, and invalid responses never fabricate receipts.
...
**Stress fixture:** Full strict parser accept/reject matrix; blank/Unicode/exact-64-MiB comments and issue/PR bodies; draft presence matrix; 64 concurrent identical calls; changed target/content conflicts; conclusive retry; every unknown trigger; malformed/missing/wrong parent/number/ID/field responses; truncated/oversize JSON; redirect/Retry-After/5xx; separate Path Sessions. Expected: exact JSON, one POST/no mutating follow-up (plus exactly one parent-kind GET for comment creation), canonical response identity, null tag/coverage, model-matching journal state, and reconciliation text.
...
**Wall budget/phase:** Always-on issue/PR creation sends one POST (maximum 30,000 ms); comment creation sends one parent-kind GET plus one POST (maximum 60,000 ms); each request is bounded by 30,000 ms. Local validation/journal/serialization budget is 150 ms at ceilings. Identical waiters share the owner's request sequence and add no request; conclusive caller retries are discrete later operations; unknown repeats return locally within 5 ms.
...
**Expected behavior:** `rfs_write` schema/tool description exposes optional `operationId` with Creation-Target-only semantics; real stdio calls return non-empty text/object-root operational errors for invalid IDs/documents, denied grants, and non-targets; source-neutral success rendering preserves response-derived canonical references, explicit null Creation Target `versionTag`, non-null Field tags, operation, and absent coverage.

**Oracle:** Checked JSON schema plus a test-local plain-text receipt parser and literal synthetic receipt/error table, independent of production render functions.

**Stress fixture:** Real binary/profile rows cover operationId absent/invalid/valid-on-non-target, malformed strict creation document, missing create grant, and every non-mutable projection without network; render-unit rows cover Field success, all three Creation Target canonical shapes, replay-equivalent success, null/non-null tags, and text/structured agreement. Expected: object-root schema, non-empty TextContent, explicit null creation tag, and operational tool errors rather than protocol errors.

**Regression fence:** `crates/resourcefs-mcp/tests/stdio_mcp_contract.rs::github_mutation_schema_and_receipts_match`; `crates/resourcefs-mcp/src/render.rs::tests::creation_receipt_renders_nullable_tag_and_canonical_reference`; existing tool-list/object-root and versioned-write rows — created/extended in this slice with `[C16]` context.

**Named mutation:** In `render.rs`, restore `skip_serializing_if` behavior for a Creation Target tag so `versionTag` is omitted rather than explicit null; the C16 render fence fails red.
...
**Wall budget/phase:** Always-on for every `rfs_write`. Local MCP overhead budget 10 ms ordinary/100 ms at maximum input; total network-backed budgets remain S5 (60,000 ms title/body; 90,000 ms comment replacement) and S6 (30,000 ms issue/PR creation; 60,000 ms comment creation).

**Files:** `crates/resourcefs-core/src/mutation.rs` for a test-support receipt constructor; `crates/resourcefs-mcp/src/{server.rs,render.rs}`; `crates/resourcefs-mcp/tests/stdio_mcp_contract.rs`; tool schema snapshots/fixtures already owned by that test surface if generated bytes change. `scripts/live-smoke.sh` and read-only live rows remain unchanged because repository policy forbids live upstream mutation.
...
**Commands and expected results:**
- `cargo test -p resourcefs-mcp --test stdio_mcp_contract github_mutation_schema_and_receipts_match` → real process schema contains optional operationId and every invalid/denied/non-target row returns the literal operational category with non-empty text; no network is attempted.
- `cargo test -p resourcefs-mcp render::tests::creation_receipt_renders_nullable_tag_and_canonical_reference` → synthetic response-derived receipts have matching text/structured semantics and explicit null Creation Target tag; the named omission mutation fails `[C16]`, then restoration is green.
...
Intended future work: rfs-cek3 already owns the reusable profile-launched fixture trust seam. Verified bearing: it adds profile-launched fixture-CA trust for deterministic positive TLS source calls; rfs-by2z's accepted direct fake and C16 split fences avoid duplicating that infrastructure.
