...
| P3 | The capability-relative rename (stable destination `Dir` handle in `RootDirectory`, simple UTF-16 name) is accepted via `NtSetInformationFile(FileRenameInformationEx)`, closing the C10 reparse-retarget race for create, replace, and move. | With that directory handle and the POSIX/replace flags, do the native Windows filesystem mutation contracts create, replace, and move successfully? | PASS |
...
- Implementation-stage P3 run: `RUSTFLAGS='-C target-feature=+crt-static' cargo xwin test -p resourcefs-sources --all-features --test filesystem_mutation_contract --target x86_64-pc-windows-msvc --no-run && ./.rfs-r9m6/windows_test_runner.py 'target/x86_64-pc-windows-msvc/debug/deps/filesystem_mutation_contract-*.exe'`
...
| P3 | The native Windows contract ran seven tests. Six mutation tests failed; every attempted create reached `SetFileInformationByHandle(FileRenameInfoEx)` and returned `ERROR_INVALID_PARAMETER` (87). The policy-before-state test, which performs no commit, passed. The same executable passes all seven tests when `RootDirectory` is null and the destination is absolute, but that form does not satisfy the approved directory-relative C10 design. | Microsoft's `FILE_RENAME_INFO` reference says a non-null `RootDirectory` may identify the directory relative to which a relative `FileName` is resolved. The observed Win32 behavior contradicts that published option on the project Windows VM. | FAIL |
...
- P3: root cause identified and fixed in code (VM re-proof pending, see Resolution). The directory-relative `RootDirectory` form is a documented NT-layer capability, not a Win32 `SetFileInformationByHandle` one — that Win32 entry point rejects a non-null `RootDirectory` with `ERROR_INVALID_PARAMETER`, exactly as observed. The fix reissues the identical `FILE_RENAME_INFORMATION` buffer and `POSIX_SEMANTICS | REPLACE_IF_EXISTS` flags through `NtSetInformationFile(FileRenameInformationEx)`, which honors the directory handle in `RootDirectory`.

## Resolution (VM-proved 2026-08-23)

- **Native VM result.** `filesystem_mutation_contract` cross-compiled for `x86_64-pc-windows-msvc` and executed on the `resourcefs-win11` VM over WinRM: **7/7 tests pass** (`create_replace_state_matrix_preserves_bytes_and_permissions`, `replacement_has_no_missing_window`, `mv_is_atomic_no_clobber_and_preserves_both_entries_on_conflict`, `mv_can_cross_workspace_roots_with_exact_independent_grants`, `concurrent_replacements_serialize_and_reject_one_stale_contender`, `exact_limit_write_budget_and_one_over_rejection`, `policy_precedes_state_and_creation_never_creates_parents`), plus 20/20 tree-cleanup stress iterations with zero failures. The 6 commit tests that previously failed at `FileRenameInfoEx` now pass; the cap-std `Dir` handle carried sufficient rights, so the documented `FILE_TRAVERSE | FILE_READ_ATTRIBUTES` fallback was not needed. P1 (replacement atomic visibility) and P2 (no-clobber move) are therefore re-proved on the shipped primitive, not only the earlier standalone-probe form.
- **Root cause (MS-doc-cited).** `FILE_RENAME_INFO` (winbase.h, for `SetFileInformationByHandle`) documents `FileName` only as an absolute path, a process-CWD-relative path, or an NTFS stream name — never a name relative to `RootDirectory`. The relative form is documented only on `FILE_RENAME_INFORMATION` (ntifs.h, for `NtSetInformationFile`/`ZwSetInformationFile`): "a relative file name … the `RootDirectory` member contains a handle to the target directory … the file name itself must be a simple file name." The SMB2 spec confirms a non-zero `RootDirectory` on an unsupported path returns `STATUS_INVALID_PARAMETER` (= `ERROR_INVALID_PARAMETER`).
- **Fix applied.** `crates/resourcefs-sources/src/filesystem_mutation.rs::windows_rename_file` now calls `NtSetInformationFile` with info class `FileRenameInformationEx` (65) via a one-file `#[link(name = "ntdll")]` extern; buffer layout, `RootDirectory` = the destination-parent `Dir` handle, and the POSIX/replace flags are unchanged. NTSTATUS is mapped to a DOS error via `RtlNtStatusToDosError` for diagnostics. Both create/replace (`platform_rename`) and move (`platform_move`) route through this one primitive.
- **Compile status.** Typechecks and links for `x86_64-pc-windows-msvc` via `cargo xwin` on the Linux dev host; the statically linked `filesystem_mutation_contract` test binary builds (`target/x86_64-pc-windows-msvc/debug/deps/filesystem_mutation_contract-*.exe`). Not yet executed on Windows.
- **Why P1/P2 were re-proved too.** Production create/replace and move all commit through `windows_rename_file`, so the earlier P1 PASS (measured with a `RootDirectory`-null probe form the production code never used) did not cover the shipped path. The VM run above exercises the shipped primitive for all three, so P1/P2/P3 are now proved against the code that ships.
- **Re-proof command (native Windows VM), for future regression.**
  `PCRE2_SYS_STATIC=1 RUSTFLAGS='-C target-feature=+crt-static' cargo xwin test -p resourcefs-sources --all-features --test filesystem_mutation_contract --target x86_64-pc-windows-msvc --no-run && ./.rfs-r9m6/windows_test_runner.py 'target/x86_64-pc-windows-msvc/debug/deps/filesystem_mutation_contract-*.exe'`
- **Unused fallback (recorded for future maintainers).** If a future cap-std change narrows the `Dir` handle's rights and `RootDirectory` is rejected, the NT contract says open `RootDirectory` with `FILE_TRAVERSE | FILE_READ_ATTRIBUTES` and let the kernel perform the relative open. Not needed as of this proof.
...
- Filed: none — P3's root cause is fixed in code on rfs-73dz itself; only native-VM re-proof of P1/P2/P3 remains, not deferred scope.
