...
#### Immutable source Facts version 1

`github://owner/repo/source/<commit-sha>/<encoded-path>/facts` addresses one exact Git path beneath the validated root tree of a full lowercase 40-character commit ID. The path is decoded exactly once, component by component; empty or dot components, absolute paths and encoded separators are invalid. No selector, branch/tag resolution, symlink following, submodule expansion, raw-content fallback or alternate provider is supported. Source Facts are read-only regardless of mutation grants.

The owned schema is version 1.0 with `kind: "github.source"` and the shared Facts envelope:

- `request` carries the requested repository, `commitSha` and decoded `path`. `observed` carries validated `commitSha`, root `treeSha`, `containingTreeSha`, `path`, `mode`, `objectType` and `objectSha`. `data` repeats that verified terminal identity, includes `sizeBytes` when supplied, and carries `content`.
- `content.state: "available"` includes `encoding: "base64"`, exact `decodedSizeBytes` and `bytesBase64`. Strict decoding reconstructs the native blob bytes; arbitrary binary content is preserved.
- `content.state: "unavailable"` uses `reason: "decoded_size_limit"` with the bounded local limit, or `reason: "acquisition_failed"` with bounded operational failure facts. It never includes stale content bytes.
- `content.state: "unsupported"` uses `reason: "non_regular_object"` for terminal directories, symlinks or submodules, or `reason: "unsupported_object_mode"` for an otherwise verified unsupported native mode.
- `upstream.repository` and `upstream.commit` retain independent observations; `upstream.trees` and optional `upstream.blobs` are keyed by native object SHA and retain body/revalidation provenance separately.

Acquisition reuses the repository/commit checks, then requests each non-recursive native tree along the exact path. Before selecting an entry, the entire tree must be complete, contain at most 1,000 unique valid names with coherent mode/type identities, and reproduce its native Git object SHA under Git's entry ordering. Truncated, oversized or malformed trees refuse the read rather than exposing a selected entry from an unverified tree. Supplied object links are confined to the configured repository/deployment, including Enterprise API prefixes, and are never followed for enrichment.

Only regular-file modes acquire a native blob. A successful available result requires matching requested/native blob identities, strict base64 including final-quantum bits, coherent supplied sizes and a matching Git blob SHA. The lower-only decoded-content bound is applied to the exact decoded length before allocation, independently of HTTP-body and final-representation bounds; equal-length base64 strings can lie on opposite sides of that bound. All repository, commit, tree and blob work shares one request/deadline/body budget. A path with `d` components spends `d + 3` physical attempts (repository, commit, `d` trees, and one blob when the terminal entry is a regular file), so the lower-only attempt ceiling bounds the deepest readable path: at the default and hard-maximum 10 attempts a regular file deeper than seven components is refused with `limit_exceeded` carrying `attempts`, never published as a partial document. Lowering `maxAttempts` lowers that depth, and a retry spent anywhere in the read lowers it by one.

After terminal metadata has been verified, a decoded-size limit or ordinary blob acquisition failure may preserve that metadata without bytes. An ordinary failure is one of the failures of that single request — an outage, a rate limit, a lapsed credential, a hidden object, a dropped connection — and its sanitized facts are published beside the identity the trees already proved. Integrity/identity/link failures, cancellation, deadline expiration, acquisition-limit overflow and generation invalidation reject the whole read; failures before verified terminal metadata do not become partial source results. Failed blob revalidation never exposes cached stale bytes, and invalidation by a concurrent mutation is reported as `source_unavailable` / `cache_generation_changed` — distinct from `upstream_unavailable`, because the retry it calls for is against the caller's own changed session rather than the provider. A 304 retains exact body identity while recording revalidation provenance.

The final complete JSON must fit the representation bound. Generation and acceptance checks after serialization precede publication. The Version Tag names those acquired JSON bytes, not the native commit/tree/blob identity; normal byte-exact artifact recovery handles display overflow without reacquisition. See [the source operating example](docs/operating.md#github-immutable-source-facts).

#### Conversation-comment Facts version 1

`pr://owner/repo/<positive-number>/comments/facts` reads the conversation-comment collection and `pr://owner/repo/<positive-number>/comments/<positive-id>/facts` reads one comment. Both are read-only owned JSON using the shared envelope and the same error vocabulary as PR Facts. `kind` is `github.conversation_comment_collection` with `schemaVersion: {"major": 1, "minor": 1}`, or `github.conversation_comment` with version 1.0. Each record retains native `kind`, decimal-string `id`, presence-aware `nodeId`, the verified parent PR identity and links, presence-aware `body`/`author`, supplied `createdAt`/`updatedAt`, and original links. The parent PR is read and verified before comment acquisition. A singular read also requires its observed comment ID to match the addressed ID; request identity is never replaced with the upstream answer. Recognizable comment links must agree with the configured deployment and the record's repository, parent and comment identity; URL userinfo is rejected. A wrong or absent required parent identity, or a number that is not a pull request, rejects the whole component. Empty optional actor values remain native facts.

The collection object is present only for the collection route. It carries `scope` (`initial` or `continuation`), `state` (`complete`, `incomplete`, or `unknown`) and `acceptedCount`, plus `localLimit` (`kind`, `bound`, optional measured `observed`), bounded sanitized `failure` facts, `continuation`, and `inconsistency` only when each is true. `scope` identifies whether this document starts at the initial page or a resumed traversal segment; `state` and `acceptedCount` describe that segment. A complete continuation segment does not claim to contain the full conversation. The canonical Resource remains the collection. `providerCap` and `reportedTotal` are absent because the provider supplies neither; a local ceiling is never presented as a provider cap. `complete` requires a fetched terminal page; local stops and ordinary later failures are incomplete. Repeated or stalled pagination and duplicate record identities cannot be silently deduplicated or reported as complete; observed inconsistent traversal is `unknown`, not a snapshot.

Pages are admitted whole and only while the record ceiling (1,000), the accepted-body ceiling, and the actual final pretty-serialized representation — including envelope, provenance, coverage, failure and continuation facts — all fit. A verified page that cannot fit is not admitted: earlier verified pages survive with honest coverage and a continuation when representable, while a first page above an admission ceiling is a typed `limit_exceeded` failure. An ordinary malformed later page or record preserves only the previously verified prefix and supplies no continuation. Cancellation, an authority/requested-identity violation, or cache-generation invalidation rejects every page. The parent and every participating response share one generation baseline, checked again after serialization. The original logical deadline covers acquisition and final acceptance: an early acquisition stop can publish a prefix only while final acceptance succeeds; actual expiry never publishes partial success.

Admission measures the observed candidate rather than reserving speculative worst-case metadata. If the actual terminal outcome subsequently grows beyond the representation ceiling, publication may remove whole verified pages until a nonempty prefix fits, with provenance and availability recomputed for that prefix and a continuation to the first removed page. It never fabricates an empty prefix or rolls back malformed/inconsistent traversal to offer a cursor; if safe publication cannot fit, the read fails with `limit_exceeded`.

A resumable truncated collection names an opaque `:cursor:<canonical-base64url>` continuation. A private random key on the session-bound source authenticates the format, configured source/API base, canonical Resource and exact native target; the public artifact session token and its digest are not signing material and are not published in the handle. Source clones share the key. Validation and origin/endpoint confinement happen before any request; tampered, foreign or prior unsigned handles acquire nothing. URL userinfo is rejected rather than interpreted as credentials. Continuation is best-effort traversal, not a snapshot. A verified but unadmitted page can be resumed at that same page, as can a target refused by a fresh-budget-recoverable limit; fetched malformed pages do not produce cursors. Limit details retain their typed dimension, bound and available measured observation, and retry guidance distinguishes relative delays from absolute times. The local record-limit dimension is `collection_records`. A caller lowers acquisition ceilings with the same `acquisition` object as PR Facts; the record ceiling remains local policy. Parent lookup, each child fetch, retry and revalidation all charge one shared budget: both comment routes, including empty collections and resumed segments, require at least two attempts. See [operating guidance](docs/operating.md#github-conversation-comment-facts).
...
| `maxDecodedBytes` | 4194304 | Decoded regular-file content bytes; immutable commit Facts and the existing PR/comment Facts perform no content decoding and retain their five-dimensional envelope |
