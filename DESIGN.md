...
Only regular-file modes acquire a native blob. A successful available result requires matching requested/native blob identities, strict base64 including final-quantum bits, coherent supplied sizes and a matching Git blob SHA. The lower-only decoded-content bound is applied to the exact decoded length before allocation, independently of HTTP-body and final-representation bounds; equal-length base64 strings can lie on opposite sides of that bound. All repository, commit, tree and blob work shares one request/deadline/body budget.

After terminal metadata has been verified, a decoded-size limit or ordinary blob acquisition failure may preserve that metadata without bytes. Integrity/identity/link failures, cancellation, deadline expiration and generation invalidation reject the whole read; failures before verified terminal metadata do not become partial source results. Failed blob revalidation never exposes cached stale bytes. A 304 retains exact body identity while recording revalidation provenance.
...
| `maxDecodedBytes` | 4194304 | Decoded regular-file content bytes; existing PR/comment Facts perform no content decoding and retain their five-dimensional envelope |
