...
| `sources/.../facts/identity.rs` | `validate`, `expected_object_url`, `require_object_link`, `validate_optional_object_link`, `validate_optional_web_link`, `require_observed_id`, `WebFragment`, `WebObjectRoute` | PR identity + reusable supplied-link authority; conversation keeps its older `validate_comment_links` here (same cluster, pre-existing) | `api_route`/`web_route` | record projection, family endpoint knowledge | `N/A` | contract tests | deepen |
