...
- C26 implementation validation — the compiled static check accepted zero/one/many-source profiles, validated paths/grants/claims/environment references/command executables, emitted byte-identical ordered `notProbed` JSON, and left the denied-network listener and helper marker untouched. Injecting `ProbeRunner` into the static branch failed the listener accept assertion; restoration passed.
- C27 implementation validation — counted fake probes covered all four states, optional/required truth-table rows, absent/present diagnostics, zero sources, and stable ordering with exactly one attempt for each compiled adapter; compiled CLI probes resolved an opaque helper credential, classified dependency failure as required `failed`, emitted the bounded redacted report on stdout with empty stderr, and mapped only optional degradation to exit 0. Injecting retry-on-failure raised the failed adapter's counter from one to two; restoration passed.
