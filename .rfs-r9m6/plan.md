...
**Oracle:** Filesystem canonical identity plus sibling sentinels (C12–C14, C16); permission lattice plus canonical-directory set (C15); test-owned normalized-extension set and explicit mode table (C11); literal read-only lattice (C16).
...
**Complexity/production scale:** Path validation is O(paths) canonicalize syscalls plus O(n) hash-set identity dedup with n ≤ 4,096 per list; extension overlap uses a normalized set, O(extensions). Production scale: 4,096 converters/roots/manifests per kind, byte-capped by the 1 MiB profile. Maximum accepted cost: under 5 seconds debug for an exact-ceiling six-kind catalog; rationale: one canonicalize per entry plus linear set operations and no recursive scanning (content validation is intended work in rfs-cdlp and rfs-btji).
...
- `cargo check -p resourcefs-sources --all-features --target x86_64-pc-windows-gnu` → the Windows adapter cross-compiles.
- `cargo test -p resourcefs-sources --target x86_64-pc-windows-gnu --all-features --test process_contract --no-run && uv run ./.rfs-r9m6/windows_test_runner.py 'target/x86_64-pc-windows-gnu/debug/deps/process_contract-*.exe'` → the runner resolves exactly one newest matching test executable, then `resourcefs-win11` reports both descendant PIDs absent (Get-Process finds neither) within ≤1,000 ms after Job termination, heartbeat sizes identical across a 300 ms window (P6 baselines: probe 60/7 bytes, oracle 54/7 bytes), and argv/environment rows matching `oracle_windows.ps1`.
