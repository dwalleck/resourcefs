...
## Implementation live record — S3 issues

- Date: 2026-09-07 UTC; isolated integration worktree `/tmp/rfs-h212-issues`, Rust 1.98.0.
- Command: `cargo +1.98.0 test -p resourcefs-sources --all-features --test jira_live_smoke live_jira_browse -- --ignored --exact --nocapture`, with the same reader-only environment and independently provisioned identities as S2.
- PASS: one real ignored row ran in 3.29 seconds, not a skipped gate. Lowered limits forced actual project offsets, site issue cursors and project issue cursors. Each family followed native continuation and reached the known fixture. The selected compact summary agreed with a separate direct `fields/summary` read; selected parent identity, canonical Resources, media type and exact-content Version Tags agreed. No tenant counts were asserted.
- Production-scale issue processing also passed with release debug assertions explicitly disabled: 1,000 rows with 4-KiB summaries processed in 22.146303 ms, within the five-second local bound, with exact ten-request/cardinality/ordering fences.
- Final fixture cleanup succeeded using the repository's marker-revalidating operator and retained receipt: `scripts/atlassian-fixture-bootstrap.sh cleanup --site <configured origin> --state <retained receipt>`. It printed `cleaned`, completed its upstream absence checks and removed the owned receipt. Credentials were not written into evidence. No live fixture remains reserved for this checkpoint.
