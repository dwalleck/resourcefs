...
The example is a tool input, not a profile fragment. Output `limits` control the rendered result, not upstream acquisition. Source and per-call acquisition policies intersect dimension by dimension; a call cannot raise the source policy. Omitted dimensions use the hard defaults before intersection: 10 attempts, 30000 ms, 8388608 response bytes, 16777216 accepted body bytes, 16777216 representation bytes, and 4194304 decoded content bytes; only a Resource that decodes content echoes the last one. Applicable HTTP substrate ceilings may lower them further. Every supplied value must be a positive integer at or below its hard ceiling. Unknown/duplicate keys, nulls and non-object controls are rejected. An empty `acquisition: {}` still explicitly requests support. Singular PR Facts; the conversation-comment, review-submission and inline review-comment collection and singular Facts; and immutable commit and source Facts support these controls (source Facts honour `maxDecodedBytes`, the others accept it and decode nothing); non-facts Resources reject them rather than ignoring them.
...
`maxDecodedBytes` lowers the decoded-content ceiling (4 MiB by default), not the HTTP-body or final-JSON budgets. Repository, commit, every traversed tree and any blob request share one acquisition budget. A path with N components costs N + 3 attempts, and a retry costs one more, so at the default (and maximum) 10 attempts a regular file deeper than seven components is refused with `limit_exceeded` on the `attempts` dimension rather than returned without content; the attempt allowance is lower-only and cannot be raised above the hard ceiling. A decoded-size limit or ordinary blob acquisition failure can return verified metadata with `content.state: "unavailable"` and a bounded reason, without stale bytes. Identity/integrity failures, cancellation, deadline expiration, acquisition-limit overflow and cache-generation changes refuse the whole read; an ordinary acquisition failure of the one blob request keeps the verified terminal metadata and reports bounded failure facts. Intermediate traversal failure cannot return a verified terminal object.
