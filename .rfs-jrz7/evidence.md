...
- **F2 (gate)**: the restored rule fails a verbatim copy placed in a counterpart that already declares the name twice (pre-repair: PASS), and the branch's own A/B harness reproduces every recorded `mutation-review3.txt` verdict in its repaired column.
