...
| core/src/acquisition.rs | 132 | 145–180 | Sixth validating dimension/getter/intersection | Existing dimension owner |
| core/src/error.rs | 293 | 295–300 | DecodedContentBytes dimension | One error type/category vocabulary |
...
| sources/src/http/read.rs | 349 | 350–355 | Preserve new validated dimension | No decoded work in transport |
| mcp/src/acquisition.rs | 151 | 165–200 | Closed field/visitor/schema/conversion | No provider types |
...
| scripts/module_shape_base.py | 548 | 560–590 | Consume explicit successor parent limits/shared-codec relocation | Historical invocation without transition remains unchanged; no filtered failures |
...
## Slice S1: Typed immutable addresses and decoded acquisition controls

**Claim IDs:** C1/C2. C6's constructor/intersection prerequisites are checked here but C6's implementation budget obligation is owned by S3; C11's final conformance is owned by S3, with mandatory shape gate at every slice.
**Expected behavior:** Public PathReference parses/canonicalizes Commit/Source exactly as approved, newtypes reject invalid values, every ResourceAddress caller is exhaustive without wildcard escape, and maxDecodedBytes validates/intersects correctly through library/profile/tool conversion. Existing adapters explicitly reject unavailable projections; no stub facts or success fallbacks.
**Oracle:** Hand-authored decoded path/invalid-input table plus independent standard-library encoding from C1; explicit arithmetic minimum table for limits. HTTP operand observation from C2 is a cross-path recheck under S3/C4, where source acquisition exists; it cannot stand in for this public parser proof.
**Stress fixture:** All C1 paths/refusals plus lowercase full SHA versus uppercase/short/ref names, %252F, U+FFFD as valid Unicode, backslash/control-byte filenames; 1024-segment bounded syntax specimen. For controls test zero/above-hard/null/negative/fractional/unknown/duplicate fields and differing caller/operator maxima; no caller can raise a bound.
**Regression fence:** core/tests/github_immutable_reference_contract.rs; extend core/tests/acquisition_contract.rs and MCP acquisition/profile/schema contracts in this slice.
**Named mutation:** C1 witness double-decodes percent literal; C2 production segment decoder double-decodes percent literal, yielding different path/refusal. Constructor bound/intersection mutation ignores operator decoded cap and fails the explicit lower-policy comparison (supporting C6 evidence).
...
**Wall budget/phase:** Always-on parsing above: <=1 second per maximum fixture; limit construction/intersection <=1 ms averaged over 10000 calls. No provider latency in this slice.
**Module shape:** Create core reference/github.rs; protected reference.rs +<=80, resource.rs +<=15, read.rs +<=11, discovery.rs +<=24, compiled.rs +<=21, mutation.rs +<=11; interfaces are the approved typed address sum and sixth limit dimension. Append the explicit successor stage and run `python scripts/module_shape.py --root . --stage immutable-grammar` → correct owner placement, shared-codec relocation and no forbidden dependencies. Preserve all prior stages and inherited obligations; pass the approved2080 reference-parent tripwire through both inherited budget checks rather than filtering their failures.
**Files:** core reference.rs/reference/github.rs/lib.rs/resource.rs/read.rs/discovery.rs/acquisition.rs/error.rs; sources compiled.rs/github/mod.rs/github/mutation.rs/http/read.rs; mcp acquisition.rs and shared schema fixture; the exact constructor callers from impact analysis; named tests; DESIGN.md/control documentation; scripts/module-ledger.json/module_shape.py; this issue artifacts. Source family is not exposed in catalog before implemented.
...
- `cargo test --locked -p resourcefs-core --test acquisition_contract` → invalid bounds refused; effective decoded limit is min(caller,operator).
...
- `cargo test --release -p resourcefs-sources --test github_facts_contract immutable_reference_parse_budget -- --ignored --exact --nocapture` → parser/limit time and tracked allocation hold; reuse that binary's existing allocator instead of adding another harness.
...
- The new decoded-content limit is non-retryable under the existing collection policy. Existing retryable caller: collection.rs:549 (LSP); no retry policy for old dimensions changes.
...
| Git tree/blob serialization, Git byte ordering, mode/type handling | No existing production implementation found; specialized gix-object/gix-hash evaluated below | Earlier existing-dependency-only audit was incomplete. Prefer evaluating the specialized object crate over hand-writing Git-format semantics; no new dependency is adopted yet. Existing base64 STANDARD remains the blob codec, not URL_SAFE_NO_PAD cursor math. |
...
- Affected functional contracts: PASS, cargo test --locked --workspace:757 passed,31 ignored (77 suites). Ignored live rows are not live proof. The expected schema-fixture failure was repaired by actual resourcefs schema output after independently checking maxDecodedBytes integer range1..4194304.
...
- Stress/production budgets: PASS, release exact65536-byte reference with1024 segments,100 iterations:225837ns average and449731 incremental heap bytes (bound2097152).65537 bytes refused.10000 control construction/intersection iterations:14ns average. Provider-latency budget N/A for this syntax/control slice.
- Regression/mutation/restoration: PASS; mutation-c1.txt, mutation-c2-parser.txt and mutation-c6-controls.txt retain compiled red and restored-green outcomes. The parser-test repair was a compile fix: ResourceAddress has no canonical_reference method; the replacement uses a fixed expected canonical identity, not a changed behavioral expectation. The redundant constructor-derived equality was removed after the existing as_str identity assertion, not because it failed.
...
| 6 Production budgets | PASS | Release parser and controls measurements above; runner recognized/executed immutable_reference_parse_budget. Network-latency phase N/A: no acquisition in S1. |
| 7 Regression fence | PASS | Parser, acquisition, profile/tool and schema contracts; full debug/release functional legs. |
| 8 Named mutation | PASS | C1/C2/C6-controls/C11 logs; each mutant changes the governed observable and compiles where applicable. |
...
Incremental review F1 is resolved by deleting the unused unbounded GithubAddress::parse API. No publication/merge of S2/S3 behavior is claimed. Final full-feature conformance, platform and live proof remain outstanding.
