...
| sources/github/facts.rs |0|450–750|Native identity/presence/owned facts|No HTTP duplication/public provider trait|
...
**Module shape:** Add private facts.rs; extend shared fetch observation/control seam, add typed Facts parser/canonical/refusal/catalog wiring. No provider JSON in core/MCP, no new client or capture coordinator. `python .rfs-0n97/oracles/module_shape.py --stage facts` → all final required owners and protected rules pass.
