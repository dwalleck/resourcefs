...
/// Maximum UTF-8 length of one Atlassian Site Mount ID.
pub const MAX_ATLASSIAN_SITE_ID_BYTES: usize = 63;
...
/// Validated lowercase identity of one configured Atlassian Site Mount.
#[derive(Debug, Clone, PartialEq, Eq, Hash, PartialOrd, Ord)]
pub struct AtlassianSiteId(String);

impl AtlassianSiteId {
    pub fn new(value: impl Into<String>) -> Result<Self, ResourceError> {
        let value = value.into();
        let bytes = value.as_bytes();
        let valid = !bytes.is_empty()
            && bytes.len() <= MAX_ATLASSIAN_SITE_ID_BYTES
        ...
        if !valid {
            return Err(invalid_reference(
                "Atlassian Site ID must be 1–63 lowercase ASCII bytes, begin and end alphanumeric, and contain only letters, digits, or internal hyphens",
            ));
        }
        Ok(Self(value))
    }
...
impl PathReference {
    pub fn parse(input: impl Into<String>) -> Result<Self, ResourceError> {
        ...
        if requested.starts_with(JIRA_PREFIX) {
            ...
            return Ok(Self {
                requested,
                address: ResourceAddress::Jira(address),
                projection,
                selector_candidate: None,
                selector_error: None,
                local_candidate: None,
...
fn remote_requested(base: String, projection: Option<&ProjectionSelector>) -> String {
...
fn parse_jira_address(input: &str) -> Result<JiraAddress, ResourceError> {
    let body = input
